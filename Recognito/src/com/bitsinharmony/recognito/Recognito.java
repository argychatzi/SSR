...
import com.bitsinharmony.recognito.utils.WavReader;
...
public class Recognito<K> {
	...
	private static final String TAG = Recognito.class.getCanonicalName();
     ...
     */
    public VoicePrint createVoicePrint(K userKey, File voiceSampleFile) throws IOException {
    	Logger.d(TAG, "Create voicePrint: " + voiceSampleFile.getName());
     ...
     * @throws IOException when an I/O exception occurs
     */
    public double[] convertFileToDoubleArray(File voiceSampleFile) throws IOException {

    	WavReader wavReader = new WavReader(voiceSampleFile.getAbsolutePath());
    	try{
    		AudioInputStream sample = AudioSystem.getAudioInputStream(voiceSampleFile);
    		AudioFormat format = sample.getFormat();
    		
    		Logger.d(TAG, "WavReader.SampleRate: " + format.getSampleRate() + " AudioFormat.SampleRate: " + format.getSampleRate());
    	...
    	if(diff > 5 * Math.ulp(0.0f)) {
    		throw new IllegalArgumentException("The sample rate for this file is different than Recognito's " +
    				"defined sample rate : [" + format.getSampleRate() + "] for the file " + voiceSampleFile.getName());
    	}
        	
        	
    	return FileHelper.readAudioInputStream(sample, wavReader);
    	} catch (UnsupportedAudioFileException e){
    		e.printStackTrace();
    	}
    	return null;
        	
//    	return FileHelper.readWavInputStream(wavReader);
    }
    ...
    public VoicePrint mergeVoiceSample(K userKey, VoicePrint voicePrint) {
    	...
    	synchronized (this) {
    		if(!universalModelWasSetByUser.get()) {
    			universalModel.merge(voicePrint);
    		}
    	}
    	original.merge(voicePrint);
     ...
     */
    public VoicePrint mergeVoiceSample(K userKey, File voiceSampleFile) throws IOException {
    ...
    public List<MatchResult<K>> identify(double[] voiceSample) {
        ...
        return identify(voicePrint);
    }
    
    public List<MatchResult<K>> identify(VoicePrint voicePrint) {
     ...
     */
    public  List<MatchResult<K>> identify(File voiceSampleFile) throws IOException {
