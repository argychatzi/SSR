...
public class FileHelper {
    ...
    public static double[] readAudioInputStream(AudioInputStream is, WavReader wavReader) 
        ...
        Logger.d(TAG, "AudioInputStream.getFrameLength: " + localIs.getFrameLength() + " WavReader.getFrameLength: " + wavReader.getFrameLength());
    ...
    public static double[] readWavInputStream(WavReader wavReader) throws IOException{    	
    	double[] audioSample = new double[wavReader.getFrameLength()];
    	byte[] buffer = new byte[8192];
    	int bytesRead = 0;
    	int offset = 0;
    	
		while((bytesRead = wavReader.read(buffer)) > -1) {
			
			int wordCount = (bytesRead / 2) + (bytesRead % 2);
			for (int i = 0; i < wordCount; i++) {
				double d = (double) byteArrayToShort(buffer, 2 * i, wavReader.isBigEndian()) / 32768;
				
				if( (offset +i) < audioSample.length){
					audioSample[offset + i] = d;
				} else{
					return audioSample;
				}
